...
awsServices: ["CloudWatch", "Systems Manager", "AWS Config", "SNS", "EC2"]
...
- Deployed two managed compliance rules:
  1.  `required-tags`: Audited all resources to enforce the presence of a `project` tag (critical for cost allocation and billing).
  2.  `ec2-volume-inuse-check`: Scanned the environment for Amazon EBS volumes not attached to any EC2 instance, preventing cloud waste and ghost billing (FinOps best practices).
- Reviewed the reporting dashboard to identify both **Compliant** and **Non-compliant** resources.

> 💡 **Portfolio Tip:** Insert a screenshot of the AWS Config Dashboard showing the "Non-compliant resources" list for the ec2-volume-inuse-check rule here.
